...
class Flat(models.Model):
    ...
    new_building = models.NullBooleanField(verbose_name="Новое здание?")
...
class Owner(models.Model):
    ...
    owner_pure_phone = PhoneNumberField("Нормальлизованный номер владельца",
                                        blank=True, db_index=True)
    owners_phonenumber = models.CharField("Номер владельца", max_length=20)
    owned_flats = models.ManyToManyField(Flat, related_name="owners",
...
class Complaint(models.Model):
    complainant = models.ForeignKey(User, verbose_name="Кто жаловался",
                                    on_delete="SET_NULL", null=True)
    complaint_flat_text = "Квартира, на которую пожаловались"
    complaint_flat = models.ForeignKey(Flat,
                                       verbose_name=complaint_flat_text,
                                       null=True, on_delete="SET_NULL")
    complaint = models.TextField("Текст жалобы", null=True)

    def __str__(self):
        return f"{self.complainant} {self.complaint_flat}"
